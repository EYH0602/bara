...
///
/// Regenerated by `scripts/embed-viewer.sh` (which runs `trunk build --release`
/// and copies the output here). The committed bytes are the accepted cost of the
/// embed-by-default UX — see `plans/stage-4-serve-live-reload.md`. CI's
/// `viewer-embed-fresh` job runs `scripts/embed-viewer.sh --check` to fail the
/// build if the frontend source changed without a matching regen, so `ara serve`
/// can't silently ship a stale UI.
